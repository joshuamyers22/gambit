...
| Tests | `uv run pytest --cov=gambit --cov-report=term-missing` | Pass | 772 passed; native tests executed locally |
...
### [Resolved] Non-date columns were silently interpreted as epoch dates

- Location: `src/gambit/holiday_calendars.py`, Polars conversion and trading-day membership
- Evidence: a Polars integer column containing `[1, 2]` produced trading-day counts exceeding 13,000 when paired with a 2024 endpoint. Boolean and duration columns were also cast into dates, and NumPy integer arrays were accepted by membership even though counting rejected them.
- Failure mode: passing the wrong data column could yield plausible calendar results for dates near 1970 instead of reporting a schema error.
- Correction implemented: shared Polars conversion requires a Date or Datetime dtype, and membership uses the same date normalization as range counting. String columns must be explicitly parsed before calendar use.
- Verification: regressions reject six non-date Polars dtypes across membership, counting, enumeration, and offsets, reject numeric/boolean NumPy membership inputs, and preserve valid Date/Datetime columns, missing dates, and millisecond/microsecond/nanosecond offset precision.

### [Resolved] Trading-day offsets silently truncated scalar fractions

- Location: `src/gambit/holiday_calendars.py`, `Calendar.add_trading_days`
- Evidence: adding `1.5` days from a Friday returned the following Monday, silently truncating the fraction. Scalar strings and booleans were also accepted, while floating-point arrays failed with a different error.
- Failure mode: malformed scheduling inputs could move a trading date by an unintended whole-day offset and yield a plausible timestamp.
...
The current evidence supports market, limit, VWAP, atomic market-roll, accounting, risk, factor-cache, and interactive research workflows covered by the 772-test suite. Stop-limit orders are retained only as a deprecated compatibility type and are deliberately rejected before execution.
...
strategy, default market mode, and opt-in conservative FIFO model do not replace the general Strategy API or
establish production-strategy performance.
FIFO contract and evidence: `documentation/architecture/fifo_queue_execution.md`
and `documentation/performance/fifo_backtest_2026-09-04.md`. Full-volume native
determinism and ledger reconciliation do not imply full-volume Python trace parity.
