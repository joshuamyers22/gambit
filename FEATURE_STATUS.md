...
| Executable portfolio targets | Experimental; base-currency exposures convert through owned price timestamps, contract multipliers, FX, explicit whole-lot rounding, and inclusive per-instrument base-currency no-trade bands into detached incremental proposals that reserve current and pending positions; shared policy admission emits detached decisions, and achieved standard exposures/risk retain accepted quantities only | P1.7 buffer overrides for required reductions, supported strategy adapter evidence, representative evidence, and quant/execution-owner approval |
