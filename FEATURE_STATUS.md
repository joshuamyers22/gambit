...
| Executable portfolio targets | Experimental; base-currency exposures convert through owned price timestamps, contract multipliers, FX, explicit whole-lot rounding, and inclusive per-instrument base-currency no-trade bands into detached incremental proposals that reserve current and pending positions; achieved standard exposures can feed existing point-in-time risk measures | P1.7 constraint rechecks, buffer overrides for required reductions, supported risk admission, representative evidence, and quant/execution-owner approval |
