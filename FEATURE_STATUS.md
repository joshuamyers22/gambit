...
| Executable portfolio targets | Experimental; base-currency exposures convert through owned price timestamps, contract multipliers, FX, explicit whole-lot rounding, and inclusive per-instrument base-currency no-trade bands into detached incremental proposals that reserve current and pending positions; required constraint reductions override buffering but still pass shared admission, which emits detached decisions and accepted-only achieved risk | P1.7 supported strategy adapter evidence, representative evidence, and quant/execution-owner approval |
