...
| Walk-forward experiment evaluation | Experimental; owned chronological splits, training-only generic/covariance/tail-risk/forecast-scalar adapters, and an existing-optimizer adapter isolate fitting, validation selection, purge gaps, held-out scoring, and durable evidence/OOS equity | P1.6 quant/research-owner approval |
| Forecast scaling and fixed combination | Experimental; Polars stages retain raw/scaled/capped values, fixed weights, rule availability, and contributions; historical scalars fit only through a declared cutoff | P2.3 historical training-only weight/correlation estimates, diversification scaling, durable contribution evidence, and quant/research-owner approval |
