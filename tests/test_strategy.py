...
def test_strategy() -> None:
    data_dir = Path(__file__).parents[1] / "examples" / "notebooks" / "data"
    aapl_file_path = data_dir / "AAPL.csv.gz"
    ibm_file_path = data_dir / "IBM.csv.gz"

    aapl_prices = pl.read_csv(aapl_file_path, try_parse_dates=True)
    ibm_prices = pl.read_csv(ibm_file_path, try_parse_dates=True)

    end_time = "2023-01-05 12:00"

    end_timestamp = np.datetime64(end_time)
    aapl_prices = aapl_prices.filter(pl.col("timestamp") <= end_timestamp).sort("timestamp")
    ibm_prices = ibm_prices.filter(pl.col("timestamp") <= end_timestamp).sort("timestamp")

    timestamps = aapl_prices["timestamp"].to_numpy().astype("M8[m]")
    ...
    def zscore_indicator(
        ...
        strategy_context: pq.StrategyContextType,
    ) -> np.ndarray:  # simple moving average
        ratio = indicators.ratio
        values = np.asarray(ratio, dtype=float)
        windows = np.lib.stride_tricks.sliding_window_view(values, 130)
        zscore = np.full(values.shape, np.nan)
        zscore[129:] = (values[129:] - windows.mean(axis=1)) / windows.std(axis=1)
        return np.nan_to_num(zscore)

    def pair_strategy_signal(
        contract_group: pq.ContractGroup,
        timestamps: np.ndarray,
        indicators: SimpleNamespace,
    ...
    def pair_entry_rule(
        ...
        _orders.append(pq.MarketOrder(contract=contract, timestamp=timestamp, qty=order_qty, reason_code=reason_code))
        return _orders
    ...
    def pair_exit_rule(
        ...
        if (curr_pos > 0 and signal_value == -1) or (curr_pos < 0 and signal_value == 1):
            ...
            _orders.append(
                pq.MarketOrder(contract=contract, timestamp=timestamp, qty=order_qty, reason_code=reason_code)
            )
        return _orders
    ...
    def get_price(
        contract: pq.Contract, timestamps: np.ndarray, i: int, strategy_context: pq.StrategyContextType
    ) -> float:
    ...
    strategy_context = SimpleNamespace(
        aapl_price=aapl_prices["c"].to_numpy(), ibm_price=ibm_prices["c"].to_numpy()
    )
    ...
    for cg, prices in [(aapl_contract_group, aapl_prices), (ibm_contract_group, ibm_prices)]:
        for column in ["o", "h", "l", "c"]:
            strategy.add_indicator(column, pq.VectorIndicator(prices[column].to_numpy()), contract_groups=[cg])

    strategy.add_indicator("ratio", pq.VectorIndicator(ratio.to_numpy()))
    ...
    strategy.add_rule(
        "pair_entry_rule",
        pair_entry_rule,
        signal_name="pair_strategy_signal",
        sig_true_values=[-2, 2],
...
def test_strategy_2() -> None:
    ...
    df = pl.DataFrame({"timestamp": timestamps.astype("datetime64[ns]"), "symbol": symbols, "price": prices})
    ...
    for cg in cgs:
        _prices = df.filter(pl.col("symbol") == cg.name).join(
            pl.DataFrame({"timestamp": timestamps.astype("datetime64[ns]")}),
            on="timestamp",
            how="right",
            coalesce=True,
        )
        strategy.add_indicator("price", pq.VectorIndicator(_prices["price"].to_numpy()), [cg])
