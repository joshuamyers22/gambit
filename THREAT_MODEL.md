...
| T04: HDF5 metadata/links/compression | Hostile HDF5 file | Exhaustion, unintended external file reads | All-column preflight before payload reads; integer metadata checks; Unicode-aware budget; hard-link-only path resolution; reject external/virtual storage and variable-length types; staged replacement/backup recovery | [Persistence](src/gambit/pq_io.py), [tests](tests/test_hdf5_hardening.py) | File opening/attributes precede checks. Metadata, filters/plugins, decompression and temporary decoding memory are not fully bounded. Writer operations require trusted paths/files. Use filesystem-restricted isolated workers. |
