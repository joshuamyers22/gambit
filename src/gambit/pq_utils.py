...
def set_defaults(
    ...
    """
    if df_float_sf is not None:
        pl.Config.set_fmt_float("mixed")
    if df_display_max_rows is not None:
...
def strtup2date(tup: Any) -> tuple[np.datetime64 | None, np.datetime64 | None]:
    ...
    if tup and type(tup) is tuple and isinstance(tup[0], str):
        return (str2date(tup[0]), str2date(tup[1]))
    return tup
...
def _polars_frequency(frequency: str) -> str:
    """Translate common pandas-style offsets to Polars duration strings."""
    aliases = {"D": "1d", "W": "1w", "M": "1mo", "H": "1h", "T": "1m", "min": "1m"}
    if frequency in aliases:
        return aliases[frequency]
    for pandas_suffix, polars_suffix in (("D", "d"), ("W", "w"), ("H", "h"), ("T", "m"), ("S", "s")):
        if frequency.endswith(pandas_suffix) and frequency[: -len(pandas_suffix)].isdigit():
            return frequency[: -len(pandas_suffix)] + polars_suffix
    return frequency


def resample_vwap(df: pl.DataFrame, sampling_frequency: str) -> pl.DataFrame | None:
    """
    ...
    return df.sort(time_col).group_by_dynamic(time_col, every=_polars_frequency(sampling_frequency)).agg(
        ((pl.col("vwap") * pl.col("v")).sum() / pl.col("v").sum()).alias("vwap")
    )
...
def resample_ts(dates: np.ndarray, values: np.ndarray, sampling_frequency: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    result = result.upsample(time_column="timestamp", every=every, maintain_order=True)
    return result["timestamp"].to_numpy(), result["value"].to_numpy()
...
def series_to_array(series: pl.Series) -> np.ndarray:
    """Convert a Polars Series to a NumPy array; otherwise return the object unchanged."""
...
def day_symbol(day_int: int | np.ndarray) -> str | np.ndarray:
    day_str = np.select(
        [day_int == 0, day_int == 1, day_int == 2, day_int == 3, day_int == 4, day_int == 5, day_int == 6],
        ["M", "Tu", "W", "Th", "F", "Sa", "Su"],
        default="",
    )
    if day_str.shape == ():
        day_str = day_str.item()
    return day_str
...
def is_newer(filename: str, ref_filename: str) -> bool:
    ...
    False
    """
    if not os.path.isfile(filename) or not os.path.isfile(ref_filename):
        return True
...
def get_empty_np_value(np_dtype: np.dtype) -> Any:
    ...
    if kind == "b":
        return False  # bool
    if kind == "i" or kind == "u":
        return 0  # signed or unsigned int
    if kind == "M":
        return np.datetime64("NaT")  # datetime
    if kind == "O" or kind == "S" or kind == "U":
        return ""  # object or string or unicode
...
def linear_interpolate(
    ...
    return np.where((a2 == a1), a1, np.where(x2 == x1, np.nan, a1 + (a2 - a1) * (x - x1) / diff))
...
def _add_stream_handler(
    logger: logging.Logger, log_level: int = logging.INFO, formatter: logging.Formatter | None = None
) -> None:
...
def in_ipython() -> bool:
    ...
    if "__IPYTHON__" in vars(builtins):
        return True
    return False


def in_debug() -> bool:
    if "PQ_DEBUG_MODE" in os.environ:
        return True
    return False
...
def get_config() -> dict[str, Any]:
    ...
    This first loads values from a config file called gambit.yml in
