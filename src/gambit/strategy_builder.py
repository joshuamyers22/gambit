...
class StrategyBuilder:
    ...
    Args:
        data: A Polars DataFrame containing columns for timestamps, indicators and signals
    ...
    def __init__(self, data: pl.DataFrame | None = None) -> None:
        if data is not None:
            assert_(len(data) > 0, "data cannot be empty")
        self.data = data
    ...
    def add_series_indicator(self, name: str, column_name: str) -> None:
        ...
        self.add_indicator(name, VectorIndicator(self.data[column_name].to_numpy()))
    ...
    def add_series_rule(
        ...
        assert_(dtype == pl.Boolean, f"{column_name} expected to be boolean, got {dtype} instead")
        if len(name) == 0:
            name = column_name
        sig_name, rule_name = f"{name}_sig", f"{name}_rule"
        self.signals.append((sig_name, VectorSignal(self.data[column_name].to_numpy()), contract_groups, None, None))
    ...
    def __call__(self) -> Strategy:
        ...
        _timestamps = (
            self.data["timestamp"].to_numpy().astype(self.timestamp_unit) if self.timestamps is None else self.timestamps
        )
