...
from gambit.pq_utils import bootstrap_ci, get_child_logger
...
LineDataType = tuple[str, pl.DataFrame, dict[Any, pl.DataFrame]]

DimensionFilterType = Callable[[pl.DataFrame, str, list[tuple[str, Any]]], np.ndarray]

DataFilterType = Callable[[pl.DataFrame, list[tuple[str, Any]]], pl.DataFrame]

StatFuncType = Callable[[pl.DataFrame, str, str, str], list[LineDataType]]

DetailDisplayType = Callable[[widgets.Widget, pl.DataFrame, bool], None]


PlotFuncType = Callable[[str, str, list[LineDataType]], list[widgets.Widget]]

DataFrameTransformFuncType = Callable[[pl.DataFrame], pl.DataFrame]

SeriesTransformFuncType = Callable[[pl.Series], pl.Series | np.ndarray]
...
def display_form(form_widgets: Sequence[widgets.Widget], debug=False) -> None:
    if not debug:
        clear_output()
    box_layout = widgets.Layout(display="flex", flex_flow="column", align_items="stretch", border="solid", width="100%")
...
class SimpleTransform:
    ...
    def __call__(self, data: pl.DataFrame) -> pl.DataFrame:
        for colname, new_colname, func in self.transforms:
            data = data.with_columns(pl.Series(new_colname, func(data[colname])))
        return data


def simple_dimension_filter(data: pl.DataFrame, dim_name: str, selected_values: list[tuple[str, Any]]) -> np.ndarray:
    """
    ...
    for name, value in selected_values:
        if value == "All":
            continue
        mask &= data[name].to_numpy() == value
    values = np.unique(data.filter(mask)[dim_name].to_numpy())
    return ["All"] + values.tolist()


def simple_data_filter(data: pl.DataFrame, selected_values: list[tuple[str, Any]]) -> pl.DataFrame:
    """
...
class MeanWithCI:
    ...
    def __call__(self, filtered_data: pl.DataFrame, xcol: str, ycol: str, zcol: str) -> list[LineDataType]:
        """
        ...
        for zvalue in zvals:
            df = filtered_data.filter(pl.col(zcol) == zvalue)
            plt_data: list[Any] = []
            for group in df.partition_by(xcol, maintain_order=True):
                x = group[xcol][0]
            ...
            line = pl.DataFrame(plt_data, schema=columns, orient="row")
...
class SimpleDetailTable:
    ...
    def __call__(self, detail_widget: widgets.Widget, data: pl.DataFrame, debug=False) -> None:
        ...
        """
        with pl.Config(tbl_rows=self.min_rows), detail_widget:
            if not debug:
                clear_output()
            if self.colnames:
                data = data[self.colnames]
            display(data)


def create_selection_dropdowns(
    dims: dict[str, str], labels: dict[str, str], update_form_func: UpdateFormFuncType
) -> dict[str, Any]:
    """
...
def on_widgets_updated(
    change: traitlets.utils.bunch.Bunch, update_form_func, selection_widgets: dict[str, widgets.Widget]
) -> None:
    """
...
class LineGraphWithDetailDisplay:
    ...
    def __init__(
        ...
        self.detail_data: dict[Any, pl.DataFrame] = {}
        self.xcol = ""
    ...
    def __call__(self, xaxis_title: str, yaxis_title: str, line_data: list[LineDataType]) -> list[widgets.Widget]:
        ...
        for line_num, (zvalue, line_df, _detail_data) in enumerate(line_data):
            x = line_df[:, 0].to_numpy()
            self.xcol = line_df.columns[0]
            y = line_df[:, 1].to_numpy()
            ...
            if hovertemplate is None:
                unique, counts = np.unique(_detail_data[self.xcol].to_numpy(), return_counts=True)
    ...
    def _on_graph_click(
        self, trace: go.Trace, points: plotly.callbacks.Points, selector: plotly.callbacks.InputDeviceState
    ) -> None:
        """
        ...
        _detail_data = _detail_data.filter(pl.col(self.xcol) == points.xs[0])
...
class TestInteractivePlot(unittest.TestCase):
    def test_interactive_plot(self):
        ...
        data = pl.DataFrame({"dte": dte, "put_call": put_call, "year": year, "delta": delta, "premium": premium})
    ...
    def transform(self, data: pl.DataFrame) -> pl.DataFrame:
        np.seterr("raise")
        return data.with_columns(pl.Series("delta_rnd", percentile_buckets(np.abs(data["delta"].to_numpy()), 10)))
